...
class ConsultarVehiculosNoDisponiblesTest {
	...
	void testConsultarVehiculosNoDisponiblesComoAdmin() throws Exception {
		ResultActions result = this.sendMantenimiento("prueba10@gmail.com", CONTRASENA);
		result.andExpect(status().isOk()) // Verificar que el código de estado es 200 OK
				.andExpect(content().contentType(RUTA));
		// Puedes agregar más aserciones para verificar el contenido de la respuesta si
		// es necesario
	...
	@Test
	@Order(3)
	void testConsultarVehiculosNoDisponiblesUnexist() throws Exception {
		ResultActions result = this.sendMantenimiento("aaaaaa", "aaaaaa");
	...
	public ResultActions sendMantenimiento(String name, String pwd) throws Exception {
		...
		RequestBuilder request = MockMvcRequestBuilders.post("/users/vehiculo").contentType(RUTA)
