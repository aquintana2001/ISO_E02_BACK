...
class CancelarReservaTest {
	...
	void testCancelarReservaComoMantenimiento () throws Exception {
		ResultActions result = this.sendCliente("prueba20@gmail.com", CONTRASENA, "63c9b79a-0ce0-4a7c-8926-a60f6a8ca999");
	    result
    ...
    void testCancelarResevaComoAdmin() throws Exception {
        ResultActions result = this.sendCliente("guillermo.santos2@alu.uclm.es", CONTRASENA, "d38fbf4f-fafa-4a57-86a5-1f927438150c");
        result
	        .andExpect(status().is4xxClientError());
	        // Debería devolver un error 4xx ya que no se proporcionó autenticación
    }
    ...
    public ResultActions sendCliente(String name,String pwd,String idReserva ) throws Exception {
		...
		RequestBuilder request = MockMvcRequestBuilders.put("/users/cancelarReserva").contentType(RUTA).content(jsoUser.toString());
